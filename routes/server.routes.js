...
router.get("/graphic-cards", paginatedResults(GraphicCard), (req, res) => {
  res.json(res.paginatedResults);
});
//GET graphic-cards count
router.get("/graphic-cards/count", getGraphicCardsCount);
// POST GraphicCards by name
router.post(
    "/graphic-cards-by-name",
    check("name", "Introduzca un nombre valido").not().isEmpty(),
    getGraphicCardsByName
);
//POST GraphicCard by name
router.post(
  "/graphic-cards/name",
  check("name", "Introduzca un nombre valido").not().isEmpty(),
  getGraphicCardByName
...
router.post(
  "/graphic-cards/new",
  check("name", "Introduzca un nombre valido").not().isEmpty(),
  check("manufacturer", "Introduzca una manufacturer valida")
    .not()
    .isEmpty()
    .isLength(7),
